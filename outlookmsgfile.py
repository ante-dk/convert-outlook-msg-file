...
def clean_header_value(header_value):
    # Define valid characters according to RFC 7230
    valid_chars = set(range(32, 127))  # VCHAR (visible ASCII)
    valid_chars.add(ord(" "))  # SP (space)
    valid_chars.add(ord("\t"))  # HTAB

    return "".join(c for c in header_value if ord(c) in valid_chars)
...
def load_message_stream(entry, is_top_level, doc):
    ...
    if "TRANSPORT_MESSAGE_HEADERS" in props:
        ...
        for header, value in headers.items():
            msg[header] = clean_header_value(value)
...
def process_attachment(msg, entry, doc):
    ...
    filename = clean_header_value(filename)
    mime_type = clean_header_value(mime_type)
